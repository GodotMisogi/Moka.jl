...
function PrognosticVars(config::GlobalConfig, mesh::Mesh; backend=KA.CPU())
    ...
    ssh = padded_array(nCells; eltype=Float64)
    normalVelocity = padded_array(nVertLevels, nEdges; eltype=Float64)
    layerThickness = padded_array(nVertLevels, nCells; eltype=Float64)
    ...
    normalVelocity[:, 1:end] .= input["normalVelocity"][:, :, 1]
    layerThickness[:, 1:end] .= input["layerThickness"][:, :, 1]
